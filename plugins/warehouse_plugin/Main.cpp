...
namespace Plugins::Warehouse
{
	...
	void LoadSettings()
	{ 
		global->allPlayers = Serializer::JsonToObject<AllPlayers>();
	...
	void UserCmdStoreItem(uint clientId, const std::wstring& param, uint base)
	{
		...
		if (cash < global->config.costPerStackStore)
		{
		...
		HkFunc(HkAddCash, clientId, -global->config.costPerStackStore);
		HkFunc(HkRemoveCargo, clientId, item.iArchID, itemCount);


		const auto whPlayerAccID = HkGetAccountIDByClientID(clientId);
		auto whPlayer = std::find_if(global->allPlayers.players.begin(), global->allPlayers.players.end(), [whPlayerAccID](const WhPlayer& player) 
			{return player.accID == whPlayerAccID;}
		);

		if (whPlayer == global->allPlayers.players.end())
		{
			auto newPlayer = WhPlayer();
			newPlayer.accID = whPlayerAccID;
